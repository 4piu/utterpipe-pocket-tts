...
utterpipe-pocket-tts models list [--json] [--data-dir <path>] [--cache-dir <path>]
...
utterpipe-pocket-tts voices list [--json] ...
utterpipe-pocket-tts voices available [--json] ...
...
comma-separated selections, and ranges. Redirected output remains a concise
human list; use `--json` for one versioned document with stable catalog numbers
and full machine-readable metadata. A row is downloadable unless it is marked
`installed`; the command does not probe the network. Downloads honor the
system proxy and normal proxy environment variables.
